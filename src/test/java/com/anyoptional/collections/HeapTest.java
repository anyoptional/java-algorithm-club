...
public class HeapTest {
    ...
    boolean verifyMaxHeap(Heap<Integer> h) {
        for (int i = 0; i < h.size(); i++) {
            ...
            if (left < h.size() && h._storage.get(i) < h._storage.get(left)) {
                return false;
            }
            if (right < h.size() && h._storage.get(i) < h._storage.get(right)) {
                return false;
            }
            if (i > 0 && h._storage.get(parent) < h._storage.get(i)) {
                return false;
            }
        }
        return true;
    ...
    public void testCreateMaxHeap() {
        ...
        assertEquals(h1._storage, Arrays.asList(7, 5, 6, 4, 2, 1, 3));
    ...
    public void testCreateMinHeap() {
        ...
        assertEquals(h1._storage, Arrays.asList(1, 2, 3, 4, 5, 6, 7));
        assertFalse(h1.isEmpty());
        assertEquals(h1.size(), 7);
        ...
        assertEquals(h2._storage, Arrays.asList(1, 3, 2, 4, 6, 7, 5));
        assertFalse(h2.isEmpty());
        assertEquals(h2.size(), 7);
    ...
    public void testCreateMaxHeapEqualnodes() {
        ...
        assertTrue(verifyMaxHeap(heap));
        assertTrue(verifyMinHeap(heap));
        assertEquals(heap._storage, Arrays.asList(1, 1, 1, 1, 1));
    }
    ...
    public void testCreateRandomMaxHeap() {
        for (int i = 1; i <= 40; i++) {
            ...
            assertTrue(isPermutation(a, h._storage));
        }
    }
    ...
    public void testRemoving() {
        ...
        assertTrue(verifyMaxHeap(h));
        assertEquals(Arrays.asList(100, 50, 70, 10, 20, 60, 65), h._storage);
        ...
        assertEquals(v1, 60);
        assertTrue(verifyMaxHeap(h));
        assertEquals(Arrays.asList(100, 50, 70, 10, 20, 65), h._storage);

        int v2 = h.remove(4);
        assertEquals(v2, 20);
        assertTrue(verifyMaxHeap(h));
        assertEquals(Arrays.asList(100, 65, 70, 10, 50), h._storage);


        int v3 = h.remove(4);
        assertEquals(v3, 50);
        assertTrue(verifyMaxHeap(h));
        assertEquals(Arrays.asList(100, 65, 70, 10), h._storage);


        int v4 = h.remove(0);
        assertEquals(v4, 100);
        assertTrue(verifyMaxHeap(h));
        assertEquals(Arrays.asList(70, 65, 10), h._storage);
        ...
        assertEquals(v5, 70);
        assertTrue(verifyMaxHeap(h));
        assertEquals(Arrays.asList(65, 10), h._storage);

        assertEquals((int) h.peek(), 65);
        int v6 = h.remove();
        assertEquals(v6, 65);
        assertTrue(verifyMaxHeap(h));
        assertEquals(Collections.singletonList(10), h._storage);

        assertEquals((int) h.peek(), 10);
        int v7 = h.remove();
        assertEquals(v7, 10);
        assertTrue(verifyMaxHeap(h));
        assertTrue(h._storage.isEmpty());
    ...
    public void testRemoveRoot() {
        ...
        assertTrue(verifyMaxHeap(h));
        assertEquals(Arrays.asList(15, 13, 9, 5, 12, 8, 7, 4, 0, 6, 2, 1), h._storage);
        assertEquals((int) h.peek(), 15);
        int v = h.remove();
        assertEquals(v, 15);
        assertTrue(verifyMaxHeap(h));
        assertEquals(Arrays.asList(13, 12, 9, 5, 6, 8, 7, 4, 0, 1, 2), h._storage);
    ...
    public void testInsert() {
        ...
        h.insert(10);
        assertTrue(verifyMaxHeap(h));
        assertEquals(Arrays.asList(15, 13, 10, 5, 12, 9, 7, 4, 0, 6, 2, 1, 8), h._storage);
    }
    ...
    public void testInsertArrayAndRemove() {
        ...
        assertEquals(Arrays.asList(9, 7, 2, 3, 5, 1), heap._storage);
